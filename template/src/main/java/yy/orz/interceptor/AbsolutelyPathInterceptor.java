...
public class AbsolutelyPathInterceptor extends HandlerInterceptorAdapter {
    public static Logger LOGGER = LoggerFactory.getLogger(AbsolutelyPathInterceptor.class);

    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        request.setAttribute("ctx", getAbsolutelypath(request));
    ...
    private String getAbsolutelypath(HttpServletRequest request) {
        ...
        } else if (request.getServerPort() == 8443) {
